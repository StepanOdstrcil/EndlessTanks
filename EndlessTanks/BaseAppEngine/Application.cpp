...
// TODO: Better!
KeyEventArgs Application::GetKeyPressed()
{
    ...
    KeyCode::Key key = KeyCode::Key::None;
    ...
    return KeyEventArgs(key, 0, KeyEventArgs::KeyState::Pressed, shift, control, alt);
}
...
int Application::Run(std::shared_ptr<Game> pGame)
{
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        {
            // Keys (test)
            KeyEventArgs keyEventArgs = GetKeyPressed();
            if (keyEventArgs.Key != KeyCode::Key::None)
            {
                gs_Window.second->OnKeyPressed(keyEventArgs);
            }
